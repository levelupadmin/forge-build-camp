...
const Community = () => {
  return (
    <SectionWrapper label="THE COMMUNITY" variant="muted">
      ...
      <div className="grid md:grid-cols-2 gap-10 items-start">
        <div>
          ...
          <p className="text-[16px] leading-[1.8] text-muted-foreground mt-4">
        ...
        <div className="hidden md:flex items-center justify-center">
          <div className="relative w-72 h-72">
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100">
              ...
              {nodes.map((n, i) => (
                <line key={`c-${i}`} x1={50} y1={50} x2={n.x} y2={n.y} stroke="hsl(var(--primary))" strokeOpacity="0.1" strokeWidth="0.4" />
              ))}
            </svg>
            ...
            {nodes.map((n, i) => (
              <motion.div
                key={i}
                className="absolute w-3.5 h-3.5 rounded-full bg-primary/50"
            ...
            <motion.div
              className="absolute w-5 h-5 rounded-full bg-primary/70 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2"
      ...
      <div className="grid md:grid-cols-3 gap-4 mt-12">
        {features.map((f, i) => (
          <motion.div
            ...
            className="bg-white border border-black/[0.06] rounded-2xl p-5 shadow-sm"
          >
